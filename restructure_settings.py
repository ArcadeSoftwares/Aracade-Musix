...
# 1. Add state variables for delete confirm dialog near settingsScreen
target_state = '            var settingsScreen by remember { mutableStateOf("Main") }\n'
if target_state in lines:
    state_index = lines.index(target_state)
    lines.insert(state_index + 1, '            var showDeleteConfirmDialog by remember { mutableStateOf(false) }\n')
    lines.insert(state_index + 2, '            var deleteConfirmText by remember { mutableStateOf("") }\n')

# Find the exact start and end of the settings Screen block inside the ModalBottomSheet
start_idx = -1
for i, line in enumerate(lines):
    if '                if (settingsScreen == "Main") {' in line:
        start_idx = i
        break

end_idx = -1
for i in range(start_idx, len(lines)):
    if '                Spacer(modifier = Modifier.height(48.dp))' in lines[i]:
        end_idx = i
        break
...
# From start_idx to end_idx, it contains `if (settingsScreen == "Main") { ... } }` and we want to replace it.
# Wait, the `Spacer` is AFTER the `Column`!
# Let's see what is right above `Spacer(modifier = Modifier.height(48.dp))`
# It is `                }` (closing Column) and `                }` (closing if Main)
# So end_idx points to `Spacer`. I will replace from start_idx to end_idx (exclusive), meaning I will replace the `if` and its closing braces. BUT I need to make sure I don't delete the `Column`'s closing brace!

# Let's just construct the exact block that goes inside the Column!
...
"""
# Note: we need to keep the closing brace of the Column!
# So end_idx points to `                Spacer(modifier = Modifier.height(48.dp))`.
# But wait, there are TWO `}` before `Spacer` in the original file:
# 1835: `                }` (closes if settingsScreen == "Main")
# 1836: `                }` (closes Column)
# 1837: `                Spacer(...)`
# We want to replace from start_idx up to line 1836 (so end_idx - 1).
# Because line 1836 `}` MUST stay to close the Column!
# Wait! In `new_content`, does `} else if (settingsScreen == "App") { ... }` close its own block? YES.
# So `new_content` ends with a block closing `}` for the `App` block!
# But does `new_content` end with a `}` to close the `Column`? NO.
# Therefore, we MUST KEEP line 1836 `}`.
# So we replace lines `start_idx` to `end_idx - 2`!
# Let's verify:
# lines[end_idx] is Spacer
# lines[end_idx-1] is `                }` (closes Column)
# lines[end_idx-2] is `                }` (closes if Main)
# So we replace `start_idx` to `end_idx-2` (which means `lines[start_idx:end_idx-1]`).
